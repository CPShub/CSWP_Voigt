function [ kntcrv ] = build_visual_knotcurve_suf_3d( uknots, vknots, num )
%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%-%
...
unum = length(uknot);
vnum = length(vknot);
linpts = zeros(3,(unum+vnum)*num);
linmesh = zeros((unum+vnum),num);
count = 1;
offset = 0;
for i = 1:length(vknot)
    linpts(:,(count-1)*num+1:count*num) = [linspace(0+offset,1-offset,num); ones(1,num)*vknot(i); zeros(1,num)];
    linmesh(count,:) = (count-1)*num+(1:num);
    count = count+1;
end

for i = 1:length(uknot)
    linpts(:,(count-1)*num+1:count*num) = [ones(1,num)*uknot(i); linspace(0+offset,1-offset,num); zeros(1,num)];
    linmesh(count,:) = (count-1)*num+(1:num);
    count = count+1;
end
