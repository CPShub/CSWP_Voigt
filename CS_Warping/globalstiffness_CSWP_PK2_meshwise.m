function [ Kglob, Rglob ] = globalstiffness_CSWP_PK2_meshwise( eltype, geo, mesh, mat, u , curtime,eps0,k0)
...
meshes = mesh.submeshes;
for kk = 1:size(meshes, 2)
    for el = 1:meshes{1, kk}.nElems                % loop over elements
        sctr = meshes{1, kk}.elNodeCnt(el,:);       % element control points index
        elDoma = meshes{1, kk}.elDoma(el,:);        % element parametric domain
        %elCpts = mesh.coords(sctr,:);     % coordinates of element control points
        elCpts0 = meshes{1, kk}.initcoords(sctr,:); % initial coordinates of el cont points
        ...
        for ipt = 1:size(gp,1)            % loop over integration points
            ...
            [N,ders] = nurbs_derivatives( gauPts,geo{1, kk}, meshes{1, kk} );
