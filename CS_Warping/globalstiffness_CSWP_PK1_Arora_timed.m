function [ Kglob, Rglob, times] = globalstiffness_CSWP_PK1_Arora_timed( eltype, geo, mesh, mat, u, curtime, eps0, k0 )
...
times_head = toc(tic_start);
% END TIME HEAD

% Setup for Timing
K_times = zeros(1, mesh.nElems * size(gp, 1));      % Time to compute K_{IJ} = EK
K_times_mat = zeros(1, mesh.nElems * size(gp, 1));  % Time to compute mat_model in {IJ}
K_times_core = zeros(1, mesh.nElems * size(gp, 1)); % Time to compute only the "Core" Difference 
c = 0;
    ...
    for ipt = 1:size(gp,1)
        ...
        % BEGIN TIME CORE
           tic_K_IJ_Core = tic;
            ...
            K_times_core(c) = toc(tic_K_IJ_Core);
            K_times(c) = toc(tic_start);
        % END TIME CORE
...
times_body = toc(tic_body);


% Time Keeping
total_time = toc(tic_start);
times.total_time = total_time;
times.K_times_mat = K_times_mat;
times.K_times = K_times;
times.K_times_core = K_times_core;
times.body_time = times_body;
