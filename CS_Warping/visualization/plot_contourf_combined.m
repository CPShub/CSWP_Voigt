function plot_contourf_combined(flag, filenameA, filenameB, options)
...
for j = 2:length(trims(1).vertices)
    for trimid = 1:4
        ...
        if trimid <= 2
            ...
            % plot using the contourf command
            %tricontourf(trivertex(:, 1), trivertex(:, 2), face, zeros(len, 1));


            X_raw = trivertex(:, 1);
            Y_raw = trivertex(:, 2);
            k = boundary(X_raw, Y_raw);
            poly_x = X_raw(k);
            poly_y = Y_raw(k);
            F = scatteredInterpolant(X_raw, Y_raw, zeros(len, 1), 'linear', 'none');
            
            gridPoints = 300; 
            x_grid = linspace(min(X_raw), max(X_raw), gridPoints);
            y_grid = linspace(min(Y_raw), max(Y_raw), gridPoints);
            [X, Y] = meshgrid(x_grid, y_grid);
            
            Z = F(X, Y);
            in = inpolygon(X, Y, poly_x, poly_y);
            Z(~in) = NaN; % Turn anything outside your shape into blank space
            
            % 6. Plot the clean contourf
            [C0, h0] = contourf(X, Y, Z, 20, 'LineColor', 'none');

            %p0 = contourf(trivertex(:, 1), trivertex(:, 2), zeros(len,1));
            ...
            %p = contourf(trivertex(:, 1), trivertex(:, 2), cdata);
            X_raw = trivertex(:, 1);
            Y_raw = trivertex(:, 2);
            k = boundary(X_raw, Y_raw);
            poly_x = X_raw(k);
            poly_y = Y_raw(k);
            F = scatteredInterpolant(X_raw, Y_raw, cdata, 'linear', 'none');
            ...
            scatter(X_raw, Y_raw, 20, cdata, "filled");
...
view(2); % Look from directly above
shading interp;
