function [nliga_return] = nliga_returns_CE( eltype, geo, mesh, mat, dbc, tbc, fout,eps0, k0, c0, dt)
...
if eltype==30
    ...
    v = zeros(5*mesh.nCpts+6, 1);% Converged chemo-mechanical solution vector
else
...
while curtime ~= 1    % get to the end
    ...
    while (err > tol) && ( nit <= maxit)
        ...
        if eltype == 30 
            if mat.index >= 310 && mat.index < 320
                % Chemo-Hyperelastic with PK2
                % TODO: implement time dependance loop
                [ k, r] = globalstiffness_CSWP_CE(eltype, geo, mesh, mat, v, c0, curtime, eps0,k0, dt);
            end
        % belongs to hyperelastic materials
